...
local AnxCalibreManagerKoreaderPlugin = WidgetContainer:extend{
    name = "anx-calibre-manager-koreader-plugin",
    is_doc_only = true,
    title = _("Login to anx-calibre-manager server"),
...
AnxCalibreManagerKoreaderPlugin.default_settings = {
    custom_server = "",
    username = nil,
    userkey = nil,
...
function AnxCalibreManagerKoreaderPlugin:init()
    ...
    if self.settings.auto_sync and Device:hasSeamlessWifiToggle() and G_reader_settings:readSetting("wifi_enable_action") ~= "turn_on" then
        ...
        logger.warn("AnxCalibreManagerKoreaderPlugin: Automatic sync has been disabled because wifi_enable_action is *not* turn_on")
    end

    self.ui.menu:registerToMainMenu(self)
end

function AnxCalibreManagerKoreaderPlugin:getSyncPeriod()
...
function AnxCalibreManagerKoreaderPlugin:onDispatcherRegisterActions()
    Dispatcher:registerAction("anx-calibre-manager-koreader-plugin_set_autosync",
        { category="string", event="AnxCalibreManagerKoreaderPluginToggleAutoSync", title=_("Set auto progress sync"), reader=true,
        args={true, false}, toggle={_("on"), _("off")},})
    Dispatcher:registerAction("anx-calibre-manager-koreader-plugin_toggle_autosync", { category="none", event="AnxCalibreManagerKoreaderPluginToggleAutoSync", title=_("Toggle auto progress sync"), reader=true,})
    Dispatcher:registerAction("anx-calibre-manager-koreader-plugin_push_progress", { category="none", event="AnxCalibreManagerKoreaderPluginPushProgress", title=_("Push progress from this device"), reader=true,})
    ...
    if not self.ui.doc_settings:has("md5_checksum") then
        ...
        if file then
            ...
            if f then
                ...
                f:close()
                local checksum = md5(content)
                self.ui.doc_settings:saveSetting("md5_checksum", checksum)
            end
        end
    end
...
function AnxCalibreManagerKoreaderPlugin:setCustomServer(server)
    logger.dbg("AnxCalibreManagerKoreaderPlugin: Setting custom server to:", server)
    self.settings.custom_server = server ~= "" and server or nil
end

function AnxCalibreManagerKoreaderPlugin:setSyncForward(strategy)
    self.settings.sync_forward = strategy
end

function AnxCalibreManagerKoreaderPlugin:setSyncBackward(strategy)
    self.settings.sync_backward = strategy
end

function AnxCalibreManagerKoreaderPlugin:setChecksumMethod(method)
    self.settings.checksum_method = method
end

function AnxCalibreManagerKoreaderPlugin:login(menu)
...
function AnxCalibreManagerKoreaderPlugin:doLogin(username, password, menu)
...
function AnxCalibreManagerKoreaderPlugin:logout(menu)
...
function AnxCalibreManagerKoreaderPlugin:getDocumentDigest()
    return self:getFileDigest()
end

function AnxCalibreManagerKoreaderPlugin:getFileDigest()
    return self.ui.doc_settings:readSetting("md5_checksum")
end



function AnxCalibreManagerKoreaderPlugin:syncToProgress(progress)
    logger.dbg("AnxCalibreManagerKoreaderPlugin: [Sync] progress to", progress)
    if self.ui.document.info.has_pages then
...
function AnxCalibreManagerKoreaderPlugin:getProgress(ensure_networking, interactive)
    ...
    local ok, err = pcall(client.get_progress,
        ...
        function(ok, body)
            ...
            local progress = self:getLastProgress()
            local percentage = self:getLastPercent()
            logger.dbg("AnxCalibreManagerKoreaderPlugin: Current progress:", percentage * 100, "% =>", progress)
...
function AnxCalibreManagerKoreaderPlugin:_onCloseDocument()
    logger.dbg("AnxCalibreManagerKoreaderPlugin: onCloseDocument")
...
function AnxCalibreManagerKoreaderPlugin:schedulePeriodicPush()
...
function AnxCalibreManagerKoreaderPlugin:_onPageUpdate(page)
    if page == nil then
        return
    end
...
function AnxCalibreManagerKoreaderPlugin:_onResume()
    logger.dbg("AnxCalibreManagerKoreaderPlugin: onResume")
...
function AnxCalibreManagerKoreaderPlugin:_onSuspend()
    logger.dbg("AnxCalibreManagerKoreaderPlugin: onSuspend")
...
function AnxCalibreManagerKoreaderPlugin:_onNetworkConnected()
    logger.dbg("AnxCalibreManagerKoreaderPlugin: onNetworkConnected")
    UIManager:scheduleIn(0.5, function()
...
function AnxCalibreManagerKoreaderPlugin:_onNetworkDisconnecting()
    logger.dbg("AnxCalibreManagerKoreaderPlugin: onNetworkDisconnecting")
    -- Network is supposed to be on already, don't wrap this in willRerunWhenOnline
    self:updateProgress(false, false)
end
...
function AnxCalibreManagerKoreaderPlugin:registerEvents()
    if self.settings.auto_sync then
